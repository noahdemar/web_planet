...
fn cloudShade(dirIn: vec3<f32>, camPos: vec3<f32>, sunDir: vec3<f32>,
  ...
  // ── volume, for the light only ──────────────────────────────────────────
  //
  // Everything above this point is untouched: the coverage, the parallax lean
  // and the opacity are the shell's, because the shell is what makes the deck
  // read from orbit and replacing it with an integrated optical depth made the
  // clouds fainter, not deeper. The march is added *underneath* that, and it
  // does exactly one job — work out how much sunlight reaches each depth.
  //
  // That is the part a single shell genuinely cannot do. The old base shadow
  // was 1 - 0.55 * thick, a proxy on the same number the opacity came from,
  // so a cloud was darker underneath simply because it was more opaque. Here
  // the light is attenuated by the cloud actually standing above it, which is
  // why towers now shade their own flanks and the deck gains relief without
  // gaining or losing any coverage.
  let vdM = normalize(dirIn * (Rg + ${CLOUD_ALT}.0) - camPos);
  let vUpM = dot(vdM, dir);
  let spanM = ${CLOUD_THICK}.0 / max(abs(vUpM), 0.30);
  // Climb, whichever side of the deck the camera is on: the shell sits at the
  // base, so from above the view ray runs down out of the slab and marching
  // along it would integrate nothing.
  let mdir = vdM * select(-1.0, 1.0, vUpM >= 0.0);
  var tauTop = 0.0;
  for (var i = 0; i < ${CLOUD_MARCH}; i = i + 1) {
    let f01 = (f32(i) + 0.5) / ${CLOUD_MARCH}.0;
    let p = dirIn * (Rg + ${CLOUD_ALT}.0) + mdir * (spanM * f01);
    let h = clamp((length(p) - (Rg + ${CLOUD_ALT}.0)) / ${CLOUD_THICK}.0, 0.0, 1.0);
    // A cumulus swells from a flat base and frays at the top rather than
    // filling the slab evenly.
    let vert = smoothstep(0.0, 0.14, h) * (1.0 - smoothstep(0.45, 1.0, h));
    let d = cloudField_C(normalize(p), t, cover, px * ${CLOUD_MARCH_PX.toFixed(4)}).x * vert;
    tauTop = tauTop + d * ${CLOUD_SIGMA.toFixed(6)} * (spanM / ${CLOUD_MARCH}.0);
  }
  ...
  // Beer's law on the cloud standing above this point, with the path through
  // it lengthened as the sun drops. A deck is flat and bright at noon and
  // deeply modelled near the terminator for exactly this reason.
  let base = exp(-tauTop * (1.4 + 2.2 * (1.0 - sunUp)));
  col = col * (0.34 + 0.66 * base);
